...
CREATE TABLE IF NOT EXISTS `cad_registry`.`metabolites` (
  ...
  `mesh_id` VARCHAR(2048) NULL COMMENT 'ncbi pubmed mesh id',
  `wikipedia` VARCHAR(2048) NULL,
  `add_time` DATETIME NOT NULL DEFAULT now(),
...
SHOW WARNINGS;
CREATE INDEX `find_name` ON `cad_registry`.`metabolites` (`name` ASC) INVISIBLE;
...
SHOW WARNINGS;
CREATE INDEX `find_name` ON `cad_registry`.`synonym` (`lang` ASC, `synonym` ASC) INVISIBLE;
