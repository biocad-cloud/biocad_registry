...
Public Module ImportsUniProt
    ...
    <Extension>
    Private Function findProtein(registry As biocad_registry, sql As IInsert, prot As entry, processNonEnzyme As Boolean) As protein
        ...
        If locus_tag.IsNullOrEmpty Then
            check = registry.protein_data.where(
                    field("source_id").in(prot.accessions),
                    field("ncbi_taxid") = taxid,
                    field("source_db") = db_uniprot) _
                .find(Of protein)("id")
        Else
            check = registry.protein_data.where(
                    field("source_id").in(locus_tag),
                    field("ncbi_taxid") = taxid,
                    field("source_db") = db_genbank) _
                .find(Of protein)("id")
        ...
        If check Is Nothing Then
            Dim uniprot_acc As String = prot.accessions.First

            Call sql.add(
                field("source_id") = uniprot_acc,
                field("ncbi_taxid") = taxid,
            ...
            If Not TypeOf sql Is CommitTransaction Then
                check = registry.protein_data.where(
                        field("source_id") = uniprot_acc,
                        field("ncbi_taxid") = taxid,
                        field("source_db") = db_uniprot) _
    ...
    Public Sub importsUniProt(registry As biocad_registry, proteins As IEnumerable(Of entry), processNonEnzyme As Boolean)
        ...
        For Each block As entry() In proteins.SplitIterator(10000)
            ...
            For Each prot As entry In TqdmWrapper.Wrap(block)
                Call registry.findProtein(sql, prot, processNonEnzyme)
            Next
