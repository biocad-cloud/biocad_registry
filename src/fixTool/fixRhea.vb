...
Module fixRhea
    ...
    Sub Main2()
        For Each filepath As String In ls - l - r - "*.owl" <= "D:\datapool\reactions\chunks"
            ...
            Dim reactions As MetabolicReaction() = loader.GetAllReactions(entity_refs:=True).ToArray
            Dim compounds As Dictionary(Of String, MetabolicCompound) = loader.GetAllCompounds.OrderBy(Function(a) a.id).ToDictionary(Function(a) a.id)

            Try
                Call filepath.debug
                Call MakeUpdates(reactions, compounds)
    ...
    Sub MakeUpdates(reactions As MetabolicReaction(), compounds As Dictionary(Of String, MetabolicCompound))
        ...
        For Each rxn As MetabolicReaction In TqdmWrapper.Wrap(reactions, bar:=bar)
            ...
            Call bar.SetLabel(rxn.ToString)
            ...
            If check Is Nothing Then
                ...
                check = registry.reaction _
                    .where(field("db_xref") = db_xref,
                           field("db_source") = rhea_key) _
                    .find(Of reaction)
